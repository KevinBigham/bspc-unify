...
# PART B — DECOMMISSION RUNBOOK ⛔ SUPERSEDED / NON-EXECUTABLE (Rulings 56 + 57)

> The entire Firebase→Supabase cutover, §B0 probe, identity backfill, and Firebase decommission below is **historical and must not be run.** Fresh Supabase launch; there is no Sitting 2. Retained for the historical record only.

*(original heading:)* **PART B — DECOMMISSION RUNBOOK (instructions-only; nothing here runs without Kevin)**
